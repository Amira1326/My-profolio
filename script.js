...
const nodemailer = require('nodemailer'); // تحتاج لتثبيت هذا: npm install nodemailer
...
// قم بإنشاء ناقل للبريد الإلكتروني باستخدام بيانات مزود الخدمة الخاص بك
let transporter = nodemailer.createTransport({
    service: 'gmail', // أو 'outlook', 'yahoo' أو إعدادات SMTP مخصصة
    auth: {
        user: 'your_email@example.com', // بريدك الإلكتروني الذي سترسل منه
        pass: 'your_email_password_or_app_password' // كلمة المرور أو كلمة مرور التطبيق إذا كنت تستخدم Gmail
    }
});

app.post('/send-email', (req, res) => {
    const { name, email, subject, message } = req.body; // احصل على البيانات من النموذج

    // تهيئة خيارات البريد الإلكتروني
    let mailOptions = {
        from: 'your_email@example.com', // بريدك الإلكتروني
        to: 'amira.alattas21@gmail.com', // البريد الإلكتروني الذي ستستقبل عليه الرسائل
    ...
    transporter.sendMail(mailOptions, (error, info) => {
        if (error) {
            ...
            // يمكنك إعادة توجيه المستخدم إلى صفحة شكر
